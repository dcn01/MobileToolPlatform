        ...
        while True:
            ...
            if "No such file or directory" not in flag_net:
                ...
                str_uid_net_stats = utils.shell(
                    "cat /proc/net/xt_qtaguid/stats|findstr %s" % utils.app_uid(
                ...
                strTotalTxBytes = utils.shell(
                    "cat proc/uid_stat/%s/tcp_snd" % utils.app_uid(self.pkg)).stdout.readline()
                strTotalRxBytes = utils.shell(
                    "cat proc/uid_stat/%s/tcp_rcv" % utils.app_uid(self.pkg)).stdout.readline()
                try:
                    tm = utils.timestamp()
