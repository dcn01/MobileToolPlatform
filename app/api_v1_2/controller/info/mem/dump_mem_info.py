...
def dumpsys_meminfo(process):
    ...
    """
    proc = subprocess.Popen(args='adb shell dumpsys mem "' + process + '"', stdout=subprocess.PIPE, stderr=sys.stderr, shell=True)
...
def execute_work(process, dir, historyFileName, abstractFileName, interval=60):
    ...
    with open(dir + "\\" + abstractFileName, "a") as write_abstract_file:
        with open(dir + "\\" + historyFileName, "a") as write_history_file:
            try:
                while True:
                    print '数据持续获取中... Press CTRL+C to break loop.'
...
if __name__ == "__main__":
    ...
    execute_work(process='com.houbank.paydayloan', dir=out_path + "\\" + dir, historyFileName=out_mem_history_file, abstractFileName=out_mem_abstract_file, interval=interval)
